...
export function snipeConfirmationHeader(params: {
  ...
  if (params.kind === "makeup") {
    if (params.discord) {
      return `A late arrow is logged under ${params.sniperLabel}. The record stands.`;
    }
    return `Makeup snipe—filed under ${params.sniperLabel}. No loose feathers.`;
  }
  return `The quarry is marked. ${params.sniperLabel} owns the shot—the count is written.`;
}
...
export function snipeConfirmationStandingsHeading(): string {
  return "Standings on the wind—now:";
}
...
export function serverNotConfigured(): string {
  return `No snipe circle drawn here. Someone with authority must stake it first.`;
}
...
export function removesnipeNothingInThread(): string {
  return `No knot to cut—already clear or the wrong fire.`;
}

export function removesnipeUndoAckEphemeral(): string {
  return `Struck from the record. Read the thread.`;
}
...
export function makeupSuccessEphemeral(): string {
  return `Written. The thread holds the full reckoning.`;
}
...
export function adjustSuccessEphemeral(): string {
  return `The books moved; the canvas knows. So be it.`;
}
...
export function leaderboardEmptyFallback(): string {
  return "_The grass is still—no shots counted yet._";
}
...
export const discordSlashDescriptions = {
  help: "The camp laws: commands, rules, how we tally honor.",
  leaderboard: "Who rides highest in the tally today?",
