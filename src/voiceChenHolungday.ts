...
export function snipeConfirmationHeader(params: {
  ...
  if (params.kind === "makeup") {
    ...
    return `Makeup snipe for ${params.sniperLabel}—filed. All above board.`;
  }
  return `That's a wrap—${params.sniperLabel} gets the credit. I've filed the boring part too.`;
}
...
export function serverNotConfigured(): string {
  return `No snipe lane configured—someone with mod keys needs to set it up first.`;
}
...
export function removesnipeNothingInThread(): string {
  return `Nothing to dismiss—wrong file or already closed.`;
}

export function removesnipeUndoAckEphemeral(): string {
  return `Undone. Particulars are in the thread.`;
}
...
export function makeupSuccessEphemeral(): string {
  return `Filed. Thread has the rest—try to keep it civil.`;
}
...
export function adjustSuccessEphemeral(): string {
  return `Adjusted. Canvas matches—let's not drag this out.`;
}
...
export function leaderboardEmptyFallback(): string {
  return "_Quiet shift—nobody on the board yet. Give it time._";
}
...
export const discordSlashDescriptions = {
  help: "Briefing pack: commands, rules, where the snipe lane is.",
  leaderboard: "Who's top of the docket today?",
