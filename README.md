...
| `poll list` / `poll check 2` / `poll Q? \| A \| B` | Same behavior as `/poll` when text fallback is on. |
...
| `/poll` | `list` / `check <n>` / `<question> \| <opt1> \| <opt2> [\| …] [\| duration]` — vote by posting only `1`, `2`, … in the poll’s thread |
...
Override names with `SLACK_LEADERBOARD_COMMAND`, `SLACK_SHOW_LEADERBOARD_COMMAND`, `SLACK_SNIPES_COMMAND`, `SLACK_HEADTOHEAD_COMMAND`, `SLACK_HELP_COMMAND`, `SLACK_POLL_COMMAND`, `SLACK_SNIPEDUEL_COMMAND`, `UNDO_COMMAND`, `MAKEUP_COMMAND`, `ADJUSTELO_COMMAND` (with or without a leading `/` in env; the bot normalizes to `/name`).
